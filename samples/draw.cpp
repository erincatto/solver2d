...
static inline RGBA8 MakeRGBA8(s2Color c)
{
...
//
s2Vec2 Camera::ConvertScreenToWorld(s2Vec2 ps)
{
	...
	float ratio = w / h;
	s2Vec2 extents = {m_zoom * ratio * 25.0f, m_zoom * 25.0f};

	s2Vec2 lower = s2Sub(m_center, extents);
	s2Vec2 upper = s2Add(m_center, extents);

	s2Vec2 pw = {(1.0f - u) * lower.x + u * upper.x, (1.0f - v) * lower.y + v * upper.y};
	return pw;
}

//
s2Vec2 Camera::ConvertWorldToScreen(s2Vec2 pw)
{
	float w = float(m_width);
	float h = float(m_height);
	float ratio = w / h;
...
struct GLRenderPoints
{
	...
	void Vertex(const s2Vec2& v, const s2Color& c, float size)
	{
	...
	void Flush()
	{
		...
		glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[0]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, m_count * sizeof(s2Vec2), m_vertices);

		glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[1]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, m_count * sizeof(s2Color), m_colors);
	...
	s2Vec2 m_vertices[e_maxVertices];
	s2Color m_colors[e_maxVertices];
...
struct GLRenderLines
{
	...
	void Vertex(const s2Vec2& v, const s2Color& c)
	{
		if (m_count == e_maxVertices)
			Flush();
...
void DrawPolygonFcn(const s2Vec2* vertices, int vertexCount, s2Color color, void* context)
{
	static_cast<Draw*>(context)->DrawPolygon(vertices, vertexCount, color);
}

void DrawSolidPolygonFcn(const s2Vec2* vertices, int vertexCount, s2Color color, void* context)
{
...
void DrawCircleFcn(s2Vec2 center, float radius, s2Color color, void* context)
{
	static_cast<Draw*>(context)->DrawCircle(center, radius, color);
}

void DrawSolidCircleFcn(s2Vec2 center, float radius, s2Vec2 axis, s2Color color, void* context)
{
	static_cast<Draw*>(context)->DrawSolidCircle(center, radius, axis, color);
}

void DrawCapsuleFcn(s2Vec2 p1, s2Vec2 p2, float radius, s2Color color, void* context)
{
	static_cast<Draw*>(context)->DrawCapsule(p1, p2, radius, color);
}

void DrawSolidCapsuleFcn(s2Vec2 p1, s2Vec2 p2, float radius, s2Color color, void* context)
{
	static_cast<Draw*>(context)->DrawSolidCapsule(p1, p2, radius, color);
}

void DrawSegmentFcn(s2Vec2 p1, s2Vec2 p2, s2Color color, void* context)
{
	static_cast<Draw*>(context)->DrawSegment(p1, p2, color);
}

void DrawTransformFcn(s2Transform xf, void* context)
{
	static_cast<Draw*>(context)->DrawTransform(xf);
}

void DrawPointFcn(s2Vec2 p, float size, s2Color color, void* context)
{
	static_cast<Draw*>(context)->DrawPoint(p, size, color);
}

void DrawStringFcn(s2Vec2 p, const char* s, void* context)
{
...
void Draw::DrawPolygon(const s2Vec2* vertices, int32_t vertexCount, s2Color color)
{
	s2Vec2 p1 = vertices[vertexCount - 1];
	for (int32_t i = 0; i < vertexCount; ++i)
	{
		s2Vec2 p2 = vertices[i];
...
void Draw::DrawSolidPolygon(const s2Vec2* vertices, int32_t vertexCount, s2Color color)
{
...
void Draw::DrawRoundedPolygon(const s2Vec2* vertices, int32_t count, float radius, s2Color fillColor, s2Color outlineColor)
{
	...
	// float inset = -S2_MAX(0, 2.0f * lineScale - radius);
	...
	{
		s2Vec2 v_prev = vertices[(i + (count - 1)) % count];
		s2Vec2 v0 = vertices[i];
		s2Vec2 v_next = vertices[(i + (count + 1)) % count];

		s2Vec2 n1 = s2Normalize(s2CrossVS(s2Sub(v0, v_prev), 1.0f));
		s2Vec2 n2 = s2Normalize(s2CrossVS(s2Sub(v_next, v0), 1.0f));
		s2Vec2 of = s2MulSV(1.0f / (s2Dot(n1, n2) + 1.0f), s2Add(n1, n2));
		s2Vec2 v = s2Add(v0, s2MulSV(inset, of));
...
void Draw::DrawCircle(s2Vec2 center, float radius, s2Color color)
{
	const float k_segments = 32.0f;
	const float k_increment = 2.0f * s2_pi / k_segments;
	float sinInc = sinf(k_increment);
	float cosInc = cosf(k_increment);
	s2Vec2 r1 = {1.0f, 0.0f};
	s2Vec2 v1 = s2MulAdd(center, radius, r1);
	for (int32_t i = 0; i < k_segments; ++i)
	{
		...
		s2Vec2 v2 = s2MulAdd(center, radius, r2);
...
void Draw::DrawSolidCircle(s2Vec2 center, float radius, s2Vec2 axis, s2Color color)
{
	s2Color fillColor = {0.5f * color.r, 0.5f * color.g, 0.5f * color.b, 0.5f};
	const float k_segments = 32.0f;
	const float k_increment = 2.0f * s2_pi / k_segments;
	...
	s2Vec2 p = s2MulAdd(center, radius, axis);
	m_lines->Vertex(center, color);
	m_lines->Vertex(p, color);
}

void Draw::DrawCapsule(s2Vec2 p1, s2Vec2 p2, float radius, s2Color color)
{
	float length;
	s2Vec2 axis = s2GetLengthAndNormalize(&length, s2Sub(p2, p1));
	...
	const float k_increment = s2_pi / k_segments;
	float sinInc = sinf(k_increment);
	float cosInc = cosf(k_increment);

	s2Vec2 r1 = {-axis.y, axis.x};
	s2Vec2 v1 = s2MulAdd(p1, radius, r1);
	s2Vec2 a = v1;
	for (int32_t i = 0; i < k_segments; ++i)
	{
		// Perform rotation to avoid additional trigonometry.
		...
		r2.x = cosInc * r1.x - sinInc * r1.y;
		r2.y = sinInc * r1.x + cosInc * r1.y;
		s2Vec2 v2 = s2MulAdd(p1, radius, r2);
		m_lines->Vertex(v1, color);
		m_lines->Vertex(v2, color);
		r1 = r2;
		v1 = v2;
	...
	for (int32_t i = 0; i < k_segments; ++i)
	{
		...
		r2.x = cosInc * r1.x - sinInc * r1.y;
		r2.y = sinInc * r1.x + cosInc * r1.y;
		s2Vec2 v2 = s2MulAdd(p2, radius, r2);
		m_lines->Vertex(v1, color);
		m_lines->Vertex(v2, color);
		r1 = r2;
		v1 = v2;
...
void Draw::DrawSolidCapsule(s2Vec2 p1, s2Vec2 p2, float radius, s2Color color)
{
	float length;
	s2Vec2 axis = s2GetLengthAndNormalize(&length, s2Sub(p2, p1));
...
void Draw::DrawSegment(s2Vec2 p1, s2Vec2 p2, s2Color color)
{
	m_lines->Vertex(p1, color);
	m_lines->Vertex(p2, color);
}

void Draw::DrawTransform(s2Transform xf)
{
	const float k_axisScale = 0.4f;
	s2Color red = {1.0f, 0.0f, 0.0f, 1.0f};
	s2Color green = {0.0f, 1.0f, 0.0f, 1.0f};
	s2Vec2 p1 = xf.p, p2;

	m_lines->Vertex(p1, red);
	p2 = s2MulAdd(p1, k_axisScale, s2Rot_GetXAxis(xf.q));
	m_lines->Vertex(p2, red);

	m_lines->Vertex(p1, green);
	p2 = s2MulAdd(p1, k_axisScale, s2Rot_GetYAxis(xf.q));
	m_lines->Vertex(p2, green);
}

void Draw::DrawPoint(s2Vec2 p, float size, s2Color color)
{
...
void Draw::DrawString(s2Vec2 pw, const char* string, ...)
{
	s2Vec2 ps = g_camera.ConvertWorldToScreen(pw);
...
void Draw::DrawAABB(s2Box aabb, s2Color c)
{
	s2Vec2 p1 = aabb.lowerBound;
	s2Vec2 p2 = {aabb.upperBound.x, aabb.lowerBound.y};
	s2Vec2 p3 = aabb.upperBound;
	s2Vec2 p4 = {aabb.lowerBound.x, aabb.upperBound.y};
